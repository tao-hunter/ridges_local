...
async def evaluate_agent_version(agent_version: AgentVersion):
    ...
    async with httpx.AsyncClient(timeout=CHALLENGE_TIMEOUT.total_seconds() * 2) as client:
        try:
            ...
            logger.info(f"Downloading agent code for agent {agent_version.agent_id} version {agent_version.version_number}")
            response = await client.get(
                f"{RIDGES_API_URL}/retrieval/agent-file",
                params={"agent_id": agent_version.agent_id},
            )
            response.raise_for_status()
            logger.info(f"Downloaded agent code for agent {agent_version.agent_id} version {agent_version.version_number}")
            ...
            logger.info(f"Saved agent code for agent {agent_version.agent_id} version {agent_version.version_number}")
            ...
            logger.error(
                f"Error configuring sandbox for agent {agent_version.agent_id} version {agent_version.version_number}: {e}"
            )
    ...
    for instance_id, patch in sbox_manager.get_successful_patches():
        prediction = {
            ...
            "model_name_or_path": f"{agent_version.agent_id}v{agent_version.version_number}",
            "model_patch": patch
        }
        ...
        if run_result:
            ...
            logger.info(f"Agent {agent_version.agent_id} version {agent_version.version_number} failed to run instance {instance_id}")
            evaluation_run = EvaluationRun(
                run_id=uuid.uuid4(),
                version_id=agent_version.version_id,
                swebench_instance_id=instance_id,

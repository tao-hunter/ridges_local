...
class EloGrader(GraderInterface):
    ...
    def get_context(self, response_1: CodegenResponse, response_2: CodegenResponse) -> str:
        return dedent(f"""
    ...
    def get_completion(self, prompt: str, context: str) -> WinLoss:
        ...
        """
        completion = openai.Client(api_key=os.getenv("OPENAI_API_KEY")).beta.chat.completions.parse(
            model="gpt-4o",
            messages=[
    ...
    def compare_responses(
        ...
        outputs = [completion.model_1_victor, completion.model_2_victor, completion.is_draw]
    ...
    def rank_elo(self, responses: List[CodegenResponse]) -> List[ValidationResult]:
        ...
        results = [ValidationResult(score=arena.get_rating(response.miner_hotkey)) for response in responses]
        return results

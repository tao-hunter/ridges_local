...
def _remote_embed(text: str, proxy_url: str, run_id:str) -> List[float]:
    ...
    for _ in range(2):  # original + 1 retry after halving
        ...
        try:
            ...
            # Try to read detailed error response from server
            try:
                error_body = http_err.read().decode('utf-8')
                error_data = json.loads(error_body)
                detail = error_data.get('detail', error_body)
                raise RuntimeError(f"embedding request failed: HTTP {http_err.code} - {detail}")
            except (json.JSONDecodeError, UnicodeDecodeError):
                # Fallback to original error if we can't parse the response
                raise RuntimeError(f"embedding request failed: HTTP {http_err.code} - {http_err.reason}")
...
def run_oneshot(
    ...
    *,
    ...
    PRE_FILTER_TOP = int(os.getenv("PREFILTER_TOP", "200"))
    ...
    MAX_WORKERS = int(os.getenv("EMBED_CONCURRENCY", "32"))
...
def inference(messages: List[Dict[str, Any]], proxy_url: str, run_id: str, model: str = None) -> dict:
    ...
    try:
        ...
        try:
            ...
            error_data = json.loads(error_body)
            detail = error_data.get('detail', error_body)
            print(f"[agent] Inference request failed: HTTP {http_err.code} - {detail}")
            raise RuntimeError(f"Inference request failed: HTTP {http_err.code} - {detail}")
        except (json.JSONDecodeError, UnicodeDecodeError):
            # Fallback to original error if we can't parse the response
            print(f"[agent] Inference request failed: HTTP {http_err.code} - {http_err.reason}")
            raise RuntimeError(f"Inference request failed: HTTP {http_err.code} - {http_err.reason}")
    except Exception as e:
